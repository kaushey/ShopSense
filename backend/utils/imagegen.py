...
def generate_images_from_json(json_data, db: Session = None, user: Users = None):
    """
    db/user are optional so this still works for anonymous/unauthenticated
    calls, but whenever both are provided each generated item is now
    persisted to the history table (previously this was written but
        ...
        if item: 
            ...
            if image_bytes:
                try:
                    ...
                    # Save to search history (one fresh row per item,
                    # only when we have an authenticated user + db session)
